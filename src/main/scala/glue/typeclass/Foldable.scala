...
trait Foldable[F[_]] {
  ...
  def combine[A](as: F[A])(implicit monoid: Monoid[A]): A = foldLeft(as)(monoid.unit)(monoid.combine)
...
trait FoldableSyntax {
  implicit class FoldableOps[F[_]: Foldable, A](self: F[A]) {
    ...
    def foldMap[B](f: A => B)(implicit monoid: Monoid[B]): B = Foldable[F].foldMap(self)(f)

    def combine(implicit monoid: Monoid[A]): A = Foldable[F].combine(self)
